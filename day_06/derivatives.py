# -*- coding: utf-8 -*-
"""
Created on Mon Jul 31 11:31:41 2023
...
def first_derivative(f, x):
    ...
    for f_index in range (1,nPts-1):
        
        dfdx[f_index] = (f[f_index+1]-f[f_index-1])/2*dx
...
def second_derivative(f, x):
    ...
    for f_index in range (1,nPts-1):
        ...
        d2fdx2[f_index] = (f[f_index+1]+f[f_index-1]-2*f[f_index])/dx**2
...
def analytic(x):
    ...
    f = 2*np.sin(x) + x**3
    dfdx = (2*np.cos(x)) + np.sin(x) + 3*x**2
    d2fdx2 = (2*np.sin(x)) + (np.cos(x)) + np.cos(x) + (6*x)

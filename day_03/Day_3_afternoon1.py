...
#the arrays have different shape one is [10,5] and second is [8,1]
...
#cannot use numpy if we are importing the package as np
...
tiegcm_dens_reshaped = np.reshape(tiegcm_dens,(nofLst_tiegcm,nofLat_tiegcm,
                                               nofAlt_tiegcm,8759), order='F')# Fortran-like index order
print(JB2008_dens_reshaped) 
...
for ik in range (5):
      cs = axs[ik].contourf(localSolarTimes_tiegcm, latitudes_tiegcm, JB2008_dens_reshaped[:,:,hi,time_array_tiegcm[ik]].squeeze().T)
